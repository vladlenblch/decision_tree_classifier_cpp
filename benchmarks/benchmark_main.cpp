...
static unsigned int g_benchmark_seed = 0;
...
Dataset create_dataset(size_t n_samples, unsigned int seed, size_t n_features = 4) {
  Dataset data;
...
static void BM_Fit_Gini_5000Samples(benchmark::State& state) {
  Dataset data = create_dataset(5000, g_benchmark_seed);
  for (auto _ : state) {
    state.PauseTiming();
